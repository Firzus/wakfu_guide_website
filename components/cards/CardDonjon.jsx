...
// Asset
import DefaultBossImage from '@public/images/defaultBossImage.png'
...
const CardDonjon = ({
    ...
    const largeScreenStyle = isLargeScreen ? { width: (midCardWidth ? midCardWidth + 'px' : sideCardWidth + 'px') } : {};

    return (
        <section
            className={`w-full select-none flex flex-col gap-2 px-4 py-2 rounded bg-black-900 whitespace-nowrap ${expend && "md:gap-4 p-4"}`}
            style={largeScreenStyle}
        >

            <Link href={`/donjons/` + link}>
                <header className={`leading-tight truncate capitalize text-white-100 md:text-lg md:font-semibold ${expend && 'text-xl font-semibold md:hidden'}`}>
                    {name}
                </header>
                <main className='flex gap-4'>
                    <figure className={`h-fit p-2 rounded-full bg-black-700 ${expend ? "md:p-4" : ""}`}>
                        <Image
                            ...
                            src={urlAPI + imageBoss.data.attributes.url}
                            width={imageBoss.data.attributes.width}
                            height={imageBoss.data.attributes.height}
                            alt={imageBoss.data.attributes.alternativeText == null && imageBoss.data.attributes.name}
                        />
                    </figure>
